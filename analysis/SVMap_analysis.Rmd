...
max_dist = 0.25       
f.sampgenos = sv %>% filter((prop_dist_filt > max_dist)) %>% distinct(MatchID)
...
matches = SV %>% distinct(ref, alt_ref, varID, svtype, gq, gt_ref_2ref) %>% mutate(match = case_when(alt_ref=="none" ~ 0, alt_ref!="none" ~ 1)) %>% group_by(ref, svtype,varID) %>% summarize(matches = sum(match)) 
