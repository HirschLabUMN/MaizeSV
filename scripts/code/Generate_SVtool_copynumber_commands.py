...
Purpose: This script generates commands for svtools copynumber, but should not need to be run under normal circumstances.  We were getting errors at the copynumber stage when using -w 100, which was fixed when -w was set to 300.  Rerunning svtools genotype is unnecessary.  However, to run svtools copynumber with a different window size, you must rerun the cnvnator_wrapper.py script from Speedseq that generates the necessary root.hist information for a given window size.  The corresponding shell script to run these commands is SVtools_Copynumber.sh.
...
for bam in os.listdir(args.b):
    if bam.endswith(".bam") and "splt" not in bam and "disc" not in bam: # We only want the full bams, not the discordant or split read bams
        ...
        if ref in vcf: # Only use the bams that pertain to the same reference genome in the merged vcf file
            cmd = ''
            if args.no_wrapper is not True: 
                cmd += "/home/hirschc1/pmonnaha/software/speedseq/bin/cnvnator_wrapper.py -t " + str(args.t) + " -w " + args.w + " -b " + args.b + bam + " -c " + "/home/hirschc1/pmonnaha/references/" + ref + "_chroms/ -T " + args.o + bam.strip(".bam") + "_tmp/cnvnator-temp -o " + args.o + bam.strip(".bam") + "_tmp/cnvnator-temp/" + bam.strip(".bam") + "_" + args.w + ".readdepth --cnvnator " + args.s + "bin/cnvnator --samtools " + args.s + "src/samtools-1.3.1/samtools && "
            cmd += "svtools copynumber --cnvnator " + args.s + "bin/cnvnator -s " + sample + " -w " + args.w + " -r " + args.o + bam.strip(".bam") + "_tmp/cnvnator-temp/" + bam + ".hist.root -c " + args.c + " -i " + args.o + "gt/" + bam.strip(".bam") + ".vcf > " + args.o + "cn/" + bam.strip(".bam") + ".vcf"
            print(cmd)
